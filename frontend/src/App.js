...
const [authLoading, setAuthLoading] = useState(false);
  ...
  // Attach Firebase ID token to API requests (required by backend verifyToken)
  useEffect(() => {
      ...
      async (config) => {
        ...
        if (firebaseUser) {
          try {
            ...
            console.warn("[auth] Failed to attach Firebase ID token:", err?.message || err);
          }
        }
        return config;
      },
  ...
  const loadPlaylists = async () => {
    try {
      ...
      toast.error(getApiErrorMessage(err, "Failed to load playlists"));
    }
  };
  ...
  const addToPlaylist = async (playlistId, song) => {
    try {
      ...
      toast.error(getApiErrorMessage(err, "Failed to add song"));
    }
  };
  ...
  const removeFromPlaylist = async (playlistId, songId) => {
    try {
      ...
      toast.error(getApiErrorMessage(err, "Failed to remove song"));
    }
  };
